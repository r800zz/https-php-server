// https://github.com/r800zz/https-php-server
...
app.use((req, res, next) => {
  ...
  const phpCgi = spawn(phpCgiPath, ['-f', filePath], { env: env });
  const chunks = [];
...
phpCgi.on('close', (code) => {
  ...
  // Remove BOM + Remove NULL characters
  ...
  if (splitIndex !== -1) {
    ...
    // If separation fails, send as-is
